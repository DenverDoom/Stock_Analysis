...
if len(sys.argv) != 3:
    print("Usage: python demo.py start_date end_date")
    sys.exit(1)

sdt = sys.argv[1]
edt = sys.argv[2]
start_date = datetime.strptime(sdt, '%Y-%m-%d').date()
end_date = datetime.strptime(edt, '%Y-%m-%d').date()
...
if con is not None:
    ...
    if cur is not None:
        ...
        if exe is not None:
            result = dbConnect.fetch_data(exe, 0)
